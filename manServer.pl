...
EOF
	print OUT qq[<$bodyTag><div id="wrap"><A NAME=top></A>\n];
...
sub processMacro
{
	...
	elsif ($c eq "TS")			# Table start
	{
		...
		$troffTable = 1;
		$troffSeparator = "\t";
		plainOutput( "<P><BLOCKQUOTE><div id='tbl'><TABLE border=1 cellspacing=0 cellpadding=3>\n" );
	}
...
sub processTable
{
	...
	if ($_ eq ".TE")
	{
		...
		flushTable();
		$troffTable = 0;
		plainOutput("</TABLE></div></BLOCKQUOTE>\n");
	}
	elsif ($_ eq ".T&")
	{
		endTblRow();
